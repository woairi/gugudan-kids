...
### 2026-03-02
- UX(게임톤)
  - 홈: "오늘 미션" 프레이밍 + 캐릭터(🐥) 말풍선 추가
  - 홈: 상단 CTA를 "바로 퀴즈 / 학습 먼저"로 크게 강조

### 2026-03-02
- UX(홈 컴팩트)
  - 홈에서 중복 메뉴 카드(학습/퀴즈) 제거 → 큰 CTA 2개만 유지
  - 스티커/보호자는 하단 아이콘 버튼 2개로 축소
  - 이어서 하기는 작은 pill 버튼으로 축소
  - 스크롤 생기지 않도록 여백/패딩 조정

### 2026-03-02
- UX(퀴즈)
  - 시작 후 상단 설정 영역 접기(집중 모드) + ⚙️ 버튼으로 열기/닫기

### 2026-03-02
- UX(퀴즈)
  - 캐릭터(🐥) 말풍선 피드백 추가(정답/오답 시 짧게 반응)

### 2026-03-02
- UX(퀴즈)
  - 진행바 강화: 문제 진행을 점(●/○)으로 표시(20문제는 10점으로 압축)

### 2026-03-02
- UX(스티커)
  - 컬렉션을 "스티커북" 3열 그리드로 변경(빈칸 채우기 느낌)

### 2026-03-02
- UX(스티커)
  - 스티커(배지) 탭 시 확대 팝업(설명/획득 여부) 추가

### 2026-03-02
- UX(스티커)
  - 스티커 팝업: 배경(오버레이) 눌러도 닫히게 개선

### 2026-03-02
- P1
  - mistakes(틀린 문제 다시 풀기) 모드 완료 시 결과 페이지 대신 "끝" 추천 화면(CTA 3개) 제공

### 2026-03-02
- UX(퀴즈)
  - 보기 버튼 터치 안전성 개선: 버튼 높이/간격 증가(h-18, gap-4)

### 2026-03-02
- UX(학습)
  - 학습 표 보기 옵션 추가(카드형/표형) + 보호자 설정에서 선택
